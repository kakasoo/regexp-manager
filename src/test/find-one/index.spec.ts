...
describe('Check regexp-builder is type-safe', () => {
    ...
    describe('include can be tuple which is length 2', () => {
        it('if you want to write include twice.', () => {
            const test = new RegExpBuilder().findOne({
                from: 'test',
                include: [
                    { partial: 'partial', options: { isForehead: true } },
                    { partial: 'partial2', options: { isForehead: false } },
                ],
            });

            expect(test).toBe('((?<=(partial))(test))(?=(partial2))');
        });
        ...
        it('if you want to write include twice.', () => {
            ...
            expect(test).toBe('((?<=(partial))(test))(?=(partial2)){1,3}');
        });
    });
